...
async fn the_next_statement_verifies_an_unverified_record() -> Result<()> {
    ...
    let report = f.import(&pool, Bank::LineBank, &paths).await?;
    assert!(report.check.ok(), "{report}");
    assert_eq!((report.inserted, report.counts.verified, report.counts.new), (1, 1, 1), "{report}");

    let (date, external_ref, reviewed): (String, String, bool) =
        sqlx::query_as("SELECT date, external_ref, reviewed FROM transactions WHERE id = ?")
            .bind(id)
            .fetch_one(&pool)
            .await?;
    ...
    assert_eq!((bal("Assets:LineBank"), bal("Expenses:Food")), (dec!(250), dec!(70)));
...
#[tokio::test]
async fn an_ambiguous_statement_leaves_the_record_unverified() -> Result<()> {
    let f = Fixture::new()?;
    let pool = frozen(&f, "ambiguous.db").await?;
    let paths =
